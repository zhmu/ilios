...
printf_uint (uint64_t i) {
	...
	/* asked to print 0? */
	if (!k) {
		/* yes. fixup for the sloppy code above :) */
		kprintf_tmp[0] = '0'; kprintf_tmp[1] = 0;
	}
}
			...
			case '%': /* formatted output */
								...
								switch (*fmt) {
										...
										case 'I': /* ipv4 address */
															...
															printf_uint ((i >> 24) & 0xff);
															j = 0;
															while (kprintf_tmp[j]) {
																if (kprintf_tmp[j] != ' ')
																	tty_putchar (kprintf_tmp[j]);
																j++;
															}
															tty_putchar ('.');
															printf_uint ((i >> 16) & 0xff);
															j = 0;
															while (kprintf_tmp[j]) {
																if (kprintf_tmp[j] != ' ')
																	tty_putchar (kprintf_tmp[j]);
