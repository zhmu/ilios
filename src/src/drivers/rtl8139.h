/*
 * rtl8139.h - ILIOS RealTek 8139 Card Driver
...
/*
 * easy port manipulation macro's
 */
#define CSR_WRITE_4(dev,csr,val) \
	outd (dev->resources.port + csr, val)
#define CSR_WRITE_2(dev,csr,val) \
	outw (dev->resources.port + csr, val)
#define CSR_WRITE_1(dev,csr,val) \
	outb (dev->resources.port + csr, val)

#define CSR_READ_4(dev,csr) \
	ind (dev->resources.port + csr)
#define CSR_READ_2(dev,csr) \
	inw (dev->resources.port + csr)
#define CSR_READ_1(dev,csr) \
	inb	 (dev->resources.port + csr)

/* EPROM stuff */
#define EE_SET(x)	\
	CSR_WRITE_1(dev, RL_EECMD, CSR_READ_1 (dev, RL_EECMD) | x)
#define EE_CLR(x)	\
	CSR_WRITE_1(dev, RL_EECMD, CSR_READ_1 (dev, RL_EECMD) & ~x)

/* MII stuff */
#define MII_SET(x) \
	CSR_WRITE_1(dev, RL_MII, CSR_READ_1 (dev, RL_MII) | (x))
#define MII_CLR(x) \
	CSR_WRITE_1(dev, RL_MII, CSR_READ_1 (dev, RL_MII) & ~(x))
...
struct RL_DATA {
	...
	char									 buffer[2048];
	addr_t                 tx_chain[RL_TX_LIST_CNT];
	size_t                 tx_len[RL_TX_LIST_CNT];
	uint8_t                last_tx;
	uint8_t                cur_tx;

	struct NETPACKET*			 last_pkt;
};
...
#define RL_CUR_TXADDR(x)        ((x->cur_tx * 4) + RL_TXADDR0)
#define RL_CUR_TXSTAT(x)        ((x->cur_tx * 4) + RL_TXSTAT0)
#define RL_CUR_TXMBUF(x)        (x->tx_chain[x->cur_tx])
#define RL_LAST_TXADDR(x)       ((x->last_tx * 4) + RL_TXADDR0)
#define RL_LAST_TXSTAT(x)       ((x->last_tx * 4) + RL_TXSTAT0)
#define RL_LAST_TXMBUF(x)       (x->tx_chain[x->rl_cdata.last_tx])
#define RL_LAST_TXMLEN(x)       (x->tx_len[x->rl_cdata.last_tx])
